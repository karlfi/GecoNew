-- Storico delle modifiche (LogTabelle): l'Operatore diventa l'utente vero.
-- I trigger di storico scrivevano SUSER_NAME(), cioe' il login SQL della connessione, che per portale,
-- motore, script e SSMS e' sempre lo stesso (twebaccount). Ora scrivono dbo.AI_Operatore(): il valore che
-- la connessione dichiara con sp_set_session_context N'Operatore' (l'API ci mette il login dell'utente del
-- portale a ogni apertura, il motore "motore: <workflow>", gli script Python "script <nome>"), altrimenti il
-- login SQL di prima (piu' il nome del PC se la connessione arriva da un'altra macchina). Chi non dichiara
-- niente scrive quindi come prima: lo script si puo' applicare prima dell'API.
-- Dei trigger cambia solo SUSER_NAME() -> dbo.AI_Operatore(); il resto e' la definizione in produzione al
-- 25/09/2026 sul DB di Ge.C.O. New (serverdb), generata da li'. TR_UP_UTENTI e' disattivato e resta com'e'.
-- Per tornare indietro: LogTabelle_operatore_ripristino.sql.
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE OR ALTER FUNCTION dbo.AI_Operatore()
RETURNS varchar(100)
AS
BEGIN
    -- chi sta scrivendo, per la colonna Operatore di LogTabelle: quello che la connessione ha dichiarato con
    -- sp_set_session_context N'Operatore' (API: il login del portale; motore e script: il loro nome),
    -- altrimenti il login SQL, con il nome del PC se la connessione non arriva dal server stesso
    DECLARE @chi nvarchar(256) = CONVERT(nvarchar(256), SESSION_CONTEXT(N'Operatore'));
    IF @chi IS NULL OR @chi = N''
        SET @chi = SUSER_NAME()
                 + CASE WHEN HOST_NAME() IS NULL OR HOST_NAME() = CONVERT(nvarchar(128), SERVERPROPERTY('MachineName'))
                        THEN N'' ELSE N' (' + HOST_NAME() + N')' END;
    RETURN LEFT(@chi, 100);
END
GO
...
begin try
  ...
  SELECT 'CLIENTI', dbo.AI_Operatore(), CASE WHEN DEL.Idcliente IS NULL THEN 'INSERT' ELSE 'UPDATE' END, XMLDOC, TAB.Idcliente
...
begin try
  insert into [LogTabelle](Tabella,Operatore,TipoOperazione,Record, IdTabella)
  SELECT 'GEO_COPERTURE', dbo.AI_Operatore(), CASE WHEN DEL.IdCopertura IS NULL THEN 'INSERT' ELSE 'UPDATE' END, XMLDOC, TAB.IdCopertura
...
begin try
  insert into [LogTabelle](Tabella,Operatore,TipoOperazione,Record, IdTabella)
  SELECT 'MEZZI', dbo.AI_Operatore(), CASE WHEN DEL.Idmezzo IS NULL THEN 'INSERT' ELSE 'UPDATE' END, XMLDOC, TAB.Idmezzo
  from dbo.MEZZI TAB
...
begin try
  insert into [LogTabelle](Tabella,Operatore,TipoOperazione,Record, IdTabella)
  SELECT 'PRODOTTI', dbo.AI_Operatore(), CASE WHEN DEL.Idprodotto IS NULL THEN 'INSERT' ELSE 'UPDATE' END, XMLDOC, TAB.Idprodotto
...
begin try
  insert into [LogTabelle](Tabella,Operatore,TipoOperazione,Record, IdTabella)
  SELECT 'SPED_AZIONI', dbo.AI_Operatore(), CASE WHEN DEL.Idazione IS NULL THEN 'INSERT' ELSE 'UPDATE' END, XMLDOC, TAB.Idazione
...
begin try
  insert into [LogTabelle](Tabella,Operatore,TipoOperazione,Record, IdTabella)
  SELECT 'SPED_STATI', dbo.AI_Operatore(), CASE WHEN DEL.idstato IS NULL THEN 'INSERT' ELSE 'UPDATE' END, XMLDOC, TAB.Idstato
...
begin try
  insert into [LogTabelle](Tabella,Operatore,TipoOperazione,Record, IdTabella)
  SELECT 'SPED_WORKFLOW', dbo.AI_Operatore(), CASE WHEN DEL.Idworkflow IS NULL THEN 'INSERT' ELSE 'UPDATE' END, XMLDOC, TAB.Idworkflow
...
begin try
  insert into [LogTabelle](Tabella,Operatore,TipoOperazione,Record, IdTabella)
  SELECT 'UTENTI', dbo.AI_Operatore(), 'DELETE', XMLDOC, DEL.Idutente
  from deleted DEL 
       cross apply (select *
                    from deleted TAB2 
...
begin try
  insert into [LogTabelle](Tabella,Operatore,TipoOperazione,Record, IdTabella)
  SELECT 'UTENTI', dbo.AI_Operatore(), CASE WHEN DEL.Idutente IS NULL THEN 'INSERT' ELSE 'UPDATE' END, XMLDOC, TAB.Idutente
  from dbo.utenti TAB
...
begin try
  insert into [LogTabelle](Tabella,Operatore,TipoOperazione,Record, IdTabella)
  SELECT 'UTENTI_ATTIVITA', dbo.AI_Operatore(), CASE WHEN DEL.IdAttivita IS NULL THEN 'INSERT' ELSE 'UPDATE' END, XMLDOC, TAB.IdAttivita
