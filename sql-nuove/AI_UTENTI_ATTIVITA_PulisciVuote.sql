...
-- dovrebbero esserci: per default quelle PRIMA della data di assunzione, oppure
-- fino a una data che si passa (@FinoAl), per svuotare una scheda doppia prima
...
CREATE OR ALTER PROCEDURE dbo.AI_UTENTI_ATTIVITA_PulisciVuote
    @IdUtente int,
    @FinoAl   date = NULL,   -- NULL = la data di assunzione della scheda
    @Conferma bit  = 0
AS
BEGIN
    SET NOCOUNT ON;

    DECLARE @DataInizio date = @FinoAl;
    IF @DataInizio IS NULL
        SELECT @DataInizio = CONVERT(date, DataInizio) FROM dbo.UTENTI WHERE IdUtente = @IdUtente;
    ...
    -- righe precedenti alla data, divise fra vuote (si possono togliere) e con
    -- qualcosa dentro (si lasciano)
    SELECT
        @IdUtente AS IdUtente,
        @DataInizio AS FinoAl,
    ...
    WHERE a.idUtente = @IdUtente AND a.data < @DataInizio;
