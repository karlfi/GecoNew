...
dati = json.load(open(BASE + r"\zone_siena.json", encoding="utf-8"))
g = dati["griglia"]
...
comune = parse_wkt(open(BASE + r"\comune_Siena.wkt", encoding="utf-8-sig").read().split("\n-----")[0])
...
NY = int((43.42 - LAT0) / SL) + 1
NX = int((11.45 - LNG0) / SG) + 1
CY0, CX0 = 0, 0
...
righe = ["SET NOCOUNT ON;",
         "DECLARE @com geometry = (SELECT SHAPE.MakeValid() FROM GEO_COMUNE WHERE DENOMINAZIONE = 'Siena');",
...
open(BASE + r"\ritaglio_si.sql", "w", encoding="utf-8").write("\n".join(righe))
json.dump(dati, open(BASE + r"\zone_siena.json", "w", encoding="utf-8"), ensure_ascii=False)
print("scritta ritaglio_si.sql")
