...
dati = json.load(open(BASE + r"\zone_scandicci.json", encoding="utf-8"))
g = dati["griglia"]
...
comune = parse_wkt(open(BASE + r"\territorio_scandicci.wkt", encoding="utf-8-sig").read().split("\n-----")[0])
...
df = pd.read_csv(BASE + r"\scandicci.csv")
df = df[(df.lat > 43.68) & (df.lat < 43.80) & (df.lng > 11.03) & (df.lng < 11.22)].copy()
...
NY = int((43.81 - LAT0) / SL) + 1
NX = int((11.23 - LNG0) / SG) + 1
CY0, CX0 = 0, 0
...
righe = ["SET NOCOUNT ON;",
         "DECLARE @com geometry = (SELECT geometry::UnionAggregate(SHAPE.MakeValid()) FROM GEO_COMUNE WHERE DENOMINAZIONE IN ('Scandicci','Lastra a Signa'));",
...
open(BASE + r"\ritaglio_sc.sql", "w", encoding="utf-8").write("\n".join(righe))
json.dump(dati, open(BASE + r"\zone_scandicci.json", "w", encoding="utf-8"), ensure_ascii=False)
print("scritta ritaglio_sc.sql")
