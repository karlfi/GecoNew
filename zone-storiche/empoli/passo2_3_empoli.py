...
dati = json.load(open(BASE + r"\zone_empoli.json", encoding="utf-8"))
g = dati["griglia"]
...
comune = parse_wkt(open(BASE + r"\territorio_empoli.wkt", encoding="utf-8-sig").read().split("\n-----")[0])
...
df = pd.read_csv(BASE + r"\empoli.csv")
df = df[(df.lat > 43.62) & (df.lat < 43.95) & (df.lng > 10.68) & (df.lng < 11.15)].copy()
...
NY = int((43.95 - LAT0) / SL) + 1
NX = int((11.15 - LNG0) / SG) + 1
CY0, CX0 = 0, 0
...
righe = ["SET NOCOUNT ON;",
         "DECLARE @com geometry = (SELECT geometry::UnionAggregate(SHAPE.MakeValid()) FROM GEO_COMUNE WHERE DENOMINAZIONE IN ('Empoli','Fucecchio','Montelupo Fiorentino','Cerreto Guidi','Vinci','Capraia e Limite'));",
...
open(BASE + r"\ritaglio_em.sql", "w", encoding="utf-8").write("\n".join(righe))
json.dump(dati, open(BASE + r"\zone_empoli.json", "w", encoding="utf-8"), ensure_ascii=False)
print("scritta ritaglio_em.sql")
