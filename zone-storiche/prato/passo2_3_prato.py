...
dati = json.load(open(BASE + r"\zone_prato.json", encoding="utf-8"))
g = dati["griglia"]
...
comune = parse_wkt(open(BASE + r"\territorio_prato.wkt", encoding="utf-8-sig").read().split("\n-----")[0])
...
df = pd.read_csv(BASE + r"\prato.csv")
df = df[(df.lat > 43.76) & (df.lat < 43.96) & (df.lng > 10.95) & (df.lng < 11.26)].copy()
...
NY = int((44.00 - LAT0) / SL) + 1
NX = int((11.31 - LNG0) / SG) + 1
CY0, CX0 = 0, 0
...
righe = ["SET NOCOUNT ON;",
         "DECLARE @com geometry = (SELECT geometry::UnionAggregate(SHAPE.MakeValid()) FROM GEO_COMUNE WHERE DENOMINAZIONE IN ('Prato','Sesto Fiorentino','Campi Bisenzio','Calenzano','Montemurlo','Poggio a Caiano','Signa'));",
...
open(BASE + r"\ritaglio_po.sql", "w", encoding="utf-8").write("\n".join(righe))
json.dump(dati, open(BASE + r"\zone_prato.json", "w", encoding="utf-8"), ensure_ascii=False)
print("scritta ritaglio_po.sql")
